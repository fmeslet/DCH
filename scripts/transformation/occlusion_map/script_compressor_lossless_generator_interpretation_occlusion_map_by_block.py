...
# Initialize an array with the same diension to store :
# the rankign of each field
...
for i, ID, idx_pkt, idx_block in zip(
    ...
    # Determine the rank of fields with the greatest impact
    # We suppress position information because we know
    # how important they are! (i.e. the sinusoids)
    pos_index_mask = []
    if (not KEEP_POS):
        idx_start = ctx_seq_ravel.shape[-1] + pkt_seq_ravel.shape[-1]
        idx_end = ctx_seq_ravel.shape[-1] + \
