...
# Initialize an array with the same diension 
# to store the ranking of each field
...
indexes = range(list_IDs_update_train_undersample.size)


# We go through each batch
for i, ID, idx_pkt, idx_block in zip(
        indexes[START_IDX:END_IDX],
        list_IDs_update_train_undersample[START_IDX:END_IDX],
        indexes_packet_update_train_undersample[START_IDX:END_IDX],
        indexes_block_update_train_undersample[START_IDX:END_IDX]):
    ...
    # To determine the ranks of the fields with the 
    # greatest impact, we remove positional information 
    # because we know it's important! (i.e. sinusoids)
    pos_index_mask = []
    if (not KEEP_POS):
        idx_start = ctx_seq_ravel.shape[-1] + pkt_seq_ravel.shape[-1]
        idx_end = ctx_seq_ravel.shape[-1] + \
    ...
    if (i % RANGE_IDX == 0):
        df_fields.to_csv(f"{RESULTS_DIR}df_FIELDS_{FULL_NAME}{EXT_NAME}_{i}.csv", 
                         index=True) # index=True to keep batch num
        # Reset only if it's not the end
        if (i != len(indexes)):
            df_fields = pd.DataFrame(
                    columns=columns_fields+columns_probas)
