...
# Context
LOOK_BACK_CONTEXT = 3 # We add +1 because we take the last packet as the one to be compressed...
LOOK_AHEAD_CONTEXT = 2 #TIMESTEPS
CONTEXT_SIZE = LOOK_BACK_CONTEXT # Nombre timesteps sur les contexts
CONTEXT_OUTPUT_SIZE = 30 #0 # Size of context in model layer
...
def get_params_compression(
    ...
    for i in range(
        ...
        # Get proba
        prob = model.predict([ctx_seq,
                              pkt_sin_seq, 
                              np.zeros((quantity, 2))], batch_size=1)
