...
RESULTS_DIR = "RESULTS/HUFFMAN/DEEP_LEARNING/"
...
DECIMALS = None #None # If None no rounding !
...
if (CHECKSUM):
    ...
    if (KEEP_ERROR):
        ...
        if (CUSTOM_SIZE is not None):
            EXT_NAME = f"_HUFFMAN_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_KEEP_ERROR_{CUT_VALUE}_DECIMALS{DECIMALS}"
        else:
            EXT_NAME = f"_HUFFMAN_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_KEEP_ERROR_{CUT_VALUE}_DECIMALS{DECIMALS}"

    else:

        if (CUSTOM_SIZE is not None):
            EXT_NAME = f"_HUFFMAN_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_{CUT_VALUE}_DECIMALS{DECIMALS}"
        else:
            EXT_NAME = f"_HUFFMAN_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_{CUT_VALUE}_DECIMALS{DECIMALS}"
    ...
    if (KEEP_ERROR):
        ...
        if (CUSTOM_SIZE is not None):
            EXT_NAME = f"_HUFFMAN_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_KEEP_ERROR_{CUT_VALUE}_DECIMALS{DECIMALS}"
        else:
            EXT_NAME = f"_HUFFMAN_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_KEEP_ERROR_{CUT_VALUE}_DECIMALS{DECIMALS}"

    else:

        if (CUSTOM_SIZE is not None):
            EXT_NAME = f"_HUFFMAN_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_{CUT_VALUE}_DECIMALS{DECIMALS}"
        else:
            EXT_NAME = f"_HUFFMAN_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_{CUT_VALUE}_DECIMALS{DECIMALS}"
...
class TableModel():
    ...
    def __init__(self, 
        ...
        Args:
            df_table (pd.DataFrame): table with context size, 
            index of the bit, keys and probabilities. 
            index_pos (np.array): array which contain index 
            position of the bits to extract.
    ...
    def predict(self, 
                ...
                ctx,
                soft=False):
        """Get the probability associated to the 
        bit to predict.

        Args:
            data (np.array): packet header.
            pos (int): bit position.
            ctx (np.array): context size.
            soft (bool, optional): if the key is unknown 
            average all the probabilities 
            associated to the bit position. Defaults to True.
...
def get_params_compression(
    ...
    for i in range(
        ...
        for j in range(
            idx_batch, idx_batch+quantity): #start_idx, end_idx): # idx_batch, idx_batch+quantity): # Verifier le end_idx (inclue ou non ?)
...
# We collect a certain number of packets
# but packets of all the selected equipment 
...
# Apply correction !
# If the number of packets is greater or equal,
...
# Index to the beginning of each block
# of each packet
...
if (CHECKSUM):
    ...
    if (KEEP_ERROR):
        ...
        if (CUSTOM_SIZE is not None):
            EXT_NAME_MODEL = f"_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_KEEP_ERROR_{CUT_VALUE}"
        else:
            EXT_NAME_MODEL = f"_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_KEEP_ERROR_{CUT_VALUE}"

    else:

        if (CUSTOM_SIZE is not None):
            EXT_NAME_MODEL = f"_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_{CUT_VALUE}"
        else:
            EXT_NAME_MODEL = f"_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_{CUT_VALUE}"
    ...
    if (KEEP_ERROR):
        ...
        if (CUSTOM_SIZE is not None):
            EXT_NAME_MODEL = f"_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_KEEP_ERROR_{CUT_VALUE}"
        else:
            EXT_NAME_MODEL = f"_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_KEEP_ERROR_{CUT_VALUE}"

    else:

        if (CUSTOM_SIZE is not None):
            EXT_NAME_MODEL = f"_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_{CUT_VALUE}"
        else:
            EXT_NAME_MODEL = f"_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_{CUT_VALUE}"
