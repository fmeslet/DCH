...
CONTEXT_SIZE = 2 # Number of timesteps on contexts
...
# For Huffman model only (also compatible herre)
SOFT_MODE = False #  True by default takes the average if the field is unknown!
DECIMALS = None # If None no rounding !
...
if (CHECKSUM):
    ...
    if (CONTEXT_IN_KEY):
        ...
        if (CUSTOM_SIZE is not None):
            EXT_NAME = f"_BASELINE_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}_CIK"
        else:
            EXT_NAME = f"_BASELINE_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}_CIK"

    else:

        if (CUSTOM_SIZE is not None):
            EXT_NAME = f"_BASELINE_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}"
        else:
            EXT_NAME = f"_BASELINE_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}"
    ...
    if (CONTEXT_IN_KEY):
        ...
        if (CUSTOM_SIZE is not None):
            EXT_NAME = f"_BASELINE_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}_CIK"
        else:
            EXT_NAME = f"_BASELINE_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}_CIK"
...
class BaselineModel():
    ...
    def predict(self, 
        ...
        Args:
            data_pkt (np.array): packets in binary format.
            data_ctx (np.array): context associated to each packet.
            pos (np.array): indexes of the bit to compress. 
            ctx (np.array): sizes of the context.
            soft (bool, optional): average the proba for the index 
            of the bit to compress if no proba is setup.
            of the bit to compress. Defaults to True.
        ...
        pred_proba = np.ones(
            (data_pkt.shape[0], self.alphabet_size))*0.5
        
        # For each block of a packet
        for i in range(data_pkt.shape[0]):
            ...
            try:
                ...
                if (CONTEXT_IN_KEY):
                    ...
                    # Concat bit context and bit
                    # from packet
                    values_extract = ctx_part + pkt_part
                    
                else:
                    values_extract = pkt_part
...
def compression_packet_parallel(
    ...
    timesteps,
    level_compress):
    """Perform packet compression in parallel.

    Args:
        prob (np.array): probabilities.
        packet (np.array): packet to compress.
        packet_target (np.array): true probabilities.
        alphabet_size (int): alphabet size.
        timesteps (int): overhead or size of the sliding window.
        level_compress (list): checkpoints to measure length of 
        the packet during compression.

    Returns:
        tuple: packet compressed and size of the packet compressed.
    """
    ...
    # If left padding we reset the
    # prediction
    if (LEFT_PADDING):
        lengths_level_compress = []
        idx_level_compress = 0
        level_compress_tmp = 0
        last_rank_bit = \
            packet.shape[0] - 1 # -1 because we count from 0 for the ranking
    else:
        level_compress_tmp = timesteps
        last_rank_bit = \
            packet.shape[0] + timesteps - 1 # -1 because we count from 0 for the ranking
    ...
    for i in range(
        ...
        # Eval compression at certain level
        if (level_compress_tmp == level_compress[
                idx_level_compress]):
            ...
            if ((idx_level_compress < len(level_compress)-1) and
                (level_compress[idx_level_compress+1] <= last_rank_bit)):
                idx_level_compress += 1
                
        # Update bit rank
        level_compress_tmp += 1 # i+timesteps
    ...
    if (LEFT_PADDING):
        ...
        lengths_level_compress = \
            np.array(lengths_level_compress) - \
                timesteps
        
    # If last level compress
    # is equal to packet length update with last bit added
    if (level_compress[
         idx_level_compress] == last_rank_bit):
        
        if (LEFT_PADDING):
            length_level_compress = len(
                    enc.data_compress)
            lengths_level_compress[-1] = \
                length_level_compress - timesteps
        else:
            length_level_compress = len(
                    enc.data_compress)
            lengths_level_compress[-1] = \
                length_level_compress        
...
def get_params_compression(
    ...
    packets_rank,
    left_padding): 
    """Get parameters for the packet compression.

    Args:
        model (tf.keras.Model): deep learning mode.
        generator (keras.utils.Sequence): Keras generator.
        headers_length (np.array): headers length.
        num_sin (int): number of sinusoids.
        look_back_context (int): size of the context.
        look_back_packet (int): size of the packet.
        max_length (int): maximum size of headers length. 
        alphabet_size (int): alphabet size.
        range_list_IDs (np.array): indexes for part inside the packet.
        range_list_IDs_tmp (np.array): indexes for part inside the packet.
        level_compress (list): checkpoints to measure length of 
        the packet during compression.
        packets_rank (np.array): rank of packet inside the flow.
        left_padding (bool): add padding to the packet 
        to compress. 

    Returns:
        tuple: parameters for packet compression.
    """
...
def compression(model,
              ...
              level_compress,
              left_padding):
    """Perform compression of packets.

    Args:
        model (tf.keras.Model): Keras model.
        packets (np.array): packets to compress.
        packets_rank (np.array): rank of packet inside the flow.
        generator (keras.utils.Sequence): Keras generator.
        headers_length (np.array): headers length.
        max_length (int): maximum size of headers length. 
        ...
        alphabet_size (int): alphabet size.
        num_sin (int): number of sinusoids.
        batch_packet (int): number of packets for each batch.
        level_compress (list): checkpoints to measure length of 
        the packet during compression.
        left_padding (bool): add padding to the packet 
        to compress. 
    ...
    Returns:
        pd.DataFrame: results of the compression.
    """
    ...
    for b in range(
        ...
        # We prepare the packets that will be compressed
        index_start = b
        ...
        with mp.Pool() as workers:
            ...
            #print("[DEGUB][compression] lengths_level_compress_tmp: ", 
            #      lengths_level_compress_tmp)
        ...
        if (LEFT_PADDING):
            packets_compress_size = \
                np.array(packets_compress_size) - \
                    look_back_packet
    ...
    if (LEFT_PADDING):
        df_results['headers_length'] = \
            (headers_length*8).astype(int) - \
                look_back_packet
    else:
        df_results['headers_length'] = \
            (headers_length*8).astype(int)
...
if ("LORA" in PROTO):
    ...
    LEVEL_COMPRESS = []
    for i in range(15, max_length, 16):
        LEVEL_COMPRESS.append(i)
...
if ((NB_EQUIP_MAX is None) or
    ...
    if ("LORA" in PROTO):
        ...
        # until the maximum number of packets is reached!
...
# We collect a certain number of packets but it 
# is necessary that packets from all the equipment 
# selected must be taken. Otherwise we sample 
# with the last if
...
# Apply correction!
# If the number of packages is greater than or equal to
# then we had to hang more to cover
# to cover all equipment
...
params = {'look_back_context': LOOK_BACK_CONTEXT,
          'look_ahead_context': LOOK_AHEAD_CONTEXT, # By default we only know with 1
          'look_back_packet': LOOK_BACK_PACKET,
          'look_ahead_packet': LOOK_AHEAD_PACKET, # By default we only know with 1
...
if (CHECKSUM):
    ...
    if (CONTEXT_IN_KEY):
        ...
        if (CUSTOM_SIZE is not None):
            EXT_NAME_MODEL = f"_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_CIK"
        else:
            EXT_NAME_MODEL = f"_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_CIK"
    ...
    if (CONTEXT_IN_KEY):
        ...
        if (CUSTOM_SIZE is not None):
            EXT_NAME_MODEL = f"_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_CIK"
        else:
            EXT_NAME_MODEL = f"_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODEL_MODE_EQUIP}_CIK"
...
df_baseline_groupby = pd.read_csv(f"{BASELINE_DIR}df_BASELINE_{FULL_NAME}{EXT_NAME_MODEL}.csv")

# Check if we use context 
# values
if (CONTEXT_IN_KEY):
    cut_value = LOOK_BACK_PACKET + LOOK_BACK_CONTEXT
else:
    cut_value = LOOK_BACK_PACKET
...
df_baseline_groupby['key'] = df_baseline_groupby['key'].map(
    lambda x : my_func(x, max_length=cut_value))
df_baseline_groupby = df_baseline_groupby.groupby(
    ['ctx', 'pos', 'key']).mean()


baseline_model = BaselineModel(df_table=df_baseline_groupby,
                               alphabet_size=ALPHABET_SIZE)
...
gc.collect()
df_results = compression(model=baseline_model,
