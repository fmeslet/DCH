...
MODE_DATASET = "checksum34" # "random", "checksum", "checksum34", "fixed", "inversion", "counter", "fixed_flow", "combinaison"
...
if (CHECKSUM):
    ...
    if (CUSTOM_SIZE is not None):
        EXT_NAME = f"_BASELINE_NAIVE_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_DATASET{MODE_DATASET}_DECIMALS{DECIMALS}"
    else:
        EXT_NAME = f"_BASELINE_NAIVE_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_DATASET{MODE_DATASET}_DECIMALS{DECIMALS}"

else:

    if (CUSTOM_SIZE is not None):
        EXT_NAME = f"_BASELINE_NAIVE_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_DATASET{MODE_DATASET}_DECIMALS{DECIMALS}"
    else:
        EXT_NAME = f"_BASELINE_NAIVE_EXTRA_SIZE{EXTRA_SIZE}_MODE_DATASET{MODE_DATASET}_DECIMALS{DECIMALS}"
...
# We shuffle for being sure that all address are covered
# step to CHECK COVERAGE become useless
