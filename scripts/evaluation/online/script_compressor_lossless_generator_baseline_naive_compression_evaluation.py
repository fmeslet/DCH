...
RESULTS_DIR = "RESULTS/BASELINE_NAIVE/"
MODELS_DIR = "MODELS/"
MAIN_DIR = "./DATA/"
...
CONTEXT_OUTPUT_SIZE = 30 # Size of context in model layer
...
MODE_EQUIP = "test" #"train" # or "test" or None for both
...
DECIMALS = None # Si None no rounding !
...
if (CHECKSUM):
    ...
    if (CUSTOM_SIZE is not None):
        EXT_NAME = f"_BASELINE_NAIVE_WITH_CHECKSUM_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}"
    else:
        EXT_NAME = f"_BASELINE_NAIVE_WITH_CHECKSUM_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}"

else:

    if (CUSTOM_SIZE is not None):
        EXT_NAME = f"_BASELINE_NAIVE_CUSTOM_SIZE{CUSTOM_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}"
    else:
        EXT_NAME = f"_BASELINE_NAIVE_EXTRA_SIZE{EXTRA_SIZE}_MODE_EQUIP{MODE_EQUIP}_DECIMALS{DECIMALS}"
    ...
    def predict(self, 
        ...
        for i in range(data_pkt.shape[0]):
            ...
            try:
                ...
                # Apply cond
                cond = (values_extract == self\
                          .df_table['key'].values)
                is_value = (self.df_table[cond].shape[0] == 0)

                if (not is_value):
                    val = self.df_table[
                        cond]['proba'].values.item()
                    #print("[DEBUG] values_extract: ", values_extract)
                    #print("[DEBUG] val: ", val)
                else:
                    if (soft):
                        val = self.df_table[
...
def compression_packet_parallel(
    ...
    timesteps,
    level_compress):
    """Compress packet in parallel.

    Args:
        prob (np.array): probabilities.
        packet (np.array): packet. 
        packet_target (): _description_
        alphabet_size (_type_): _description_
        timesteps (_type_): _description_
        level_compress (_type_): _description_
...
def get_params_compression(
    ...
    print("[DEBUG] batchs: ", batchs)
    print("[DEBUG] len(params_compression): ", 
              len(params_compression))
    print("[DEBUG] idx_batch: ", idx_batch)
    ...
    for i in range(
        ...
        for j in range(
            idx_batch, idx_batch+quantity): # Verifier le end_idx (inclue ou non ?)
...
class DataGeneratorContinuous(keras.utils.Sequence):
    ...
    def __get_context(
        ...
        # Repeat the context depending on the header length
...
df_baseline = pd.read_csv(f"{BASELINE_DIR}df_BASELINE_NAIVE_{FULL_NAME}{EXT_NAME_MODEL}.csv")
...
df_baseline['key'] = df_baseline['key'].map(
    lambda x : my_func(x, max_length=cut_value))


baseline_model = BaselineNaiveModel(df_table=df_baseline,
                                    alphabet_size=ALPHABET_SIZE)
