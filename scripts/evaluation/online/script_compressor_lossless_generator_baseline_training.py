...
# Define block size
min_block_size = \
    np.min(indexes_block_update_train)
max_block_size = \
    np.max(indexes_block_update_train)
...
for j in range(
    ...
    if (j == LOOK_BACK_CONTEXT):
        idx_packets_rank = np.where(
            packets_rank >= j)[0]
    else:
        idx_packets_rank = np.where(
            packets_rank == j)[0]
    
    # Get packets indexes belong 
    # to selected indexes (hold 
    # to the conte)
    mask_indexes_update_train = \
        np.isin(indexes_packet_update_train, 
                idx_packets_rank)
    
    # Get indexes for generator
    indexes_update_train = np.arange(
        0, indexes_packet_update_train.size, 
        dtype=int)
    
    indexes_update_train_tmp = \
        indexes_update_train\
            [mask_indexes_update_train]
    indexes_pkt_tmp = \
    ...
    # For each pos (max number of block in 
    # each packet)
    for i in range(max_block_size+1):
    
        # Select index for 
        # packet with specific rank
        indexes_tmp = \
            np.where(indexes_block_tmp == i)[0]
        print("[DEBUG] indexes_tmp: ", indexes_tmp)
        
        # Get index get item
        indexes = \
            indexes_update_train_tmp[indexes_tmp]
        ...
        for k in indexes:
            ...
            if (CONTEXT_IN_KEY):
                ...
                # Concat bit context et bit
                # form packet
                key = ctx_part + pkt_part
            else:
                key = pkt_part
        ...
        if (CONTEXT_IN_KEY):
            df_baseline_name = \
                df_baseline_name + f"_CIK"

        # Set LEFT_PADDING
        if (LEFT_PADDING):
            df_baseline_name = \
                df_baseline_name + "_LEFT_PADDING"

        df_baseline_name = \
            df_baseline_name + f"_{j}_{i}.csv"

        # Save dataframe
        df_baseline.to_csv(df_baseline_name, index=False)
